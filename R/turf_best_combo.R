...
#' @param labels Optional dictionary data frame.
  ...
  if(!is.null(subgroups)){
    ...
    if(parallel_sg){
      sg_totals <- sg_estimates %>%
        ...
        dplyr::summarise(sg_total = sum(.data$est_sec), .groups = "drop")
      total_sec <- max(sg_totals$sg_total) + 2
      sg_mode <- "parallel"
    } else {
...
.turf_core <- function(
  ...
  if(!is.null(labels)){
    ...
    if(is.character(labels)){
      labels <- .turf_dictionary_from_named(labels)
    }
